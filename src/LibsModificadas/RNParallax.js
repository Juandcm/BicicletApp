...
import { NavigationBarContainer } from "../../ArreglandoVistas/EstilosArreglandoVistas"
...
class RNParallax extends Component {
    ...
    renderBackgroundImage() {
        const { backgroundImage, Transitionfluida } = this.props;
        const imageOpacity = this.getImageOpacity();
        const imageTranslate = this.getImageTranslate();
        const imageScale = this.getImageScale();

        return (
            <Transition shared={Transitionfluida}>
                <View style={{height: this.getHeaderMaxHeight(),width:null}}>
                    <Animated.Image
                        style={[
    ...
    renderNavbarBackground() {
        const { navbarColor, headerNavbarStyle } = this.props;
    ...
    renderHeaderTitle() {
        const { headerTitleStyle, renderContentTitle, } = this.props;
    ...
    renderHeaderForeground() {
        const { renderNavBar, iconoHeader, tipoNavegacion, textNavegacion } = this.props;
        ...
        return (
            ...
            >
                <NavigationBarContainer>
                    <View style={{
                        marginLeft: 10, width: wp('80%'), justifyContent: 'space-between',
                        alignItems: 'center',
                        flexDirection: 'row',
                    }}>
                        <TouchableHighlight
                        ...
                        >
                            <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', }} >
                                <Ionicons
                                    ...
                                    style={{ zIndex: 90 }}
                                />
                                <CustomText style={{ zIndex: 1, color: '#fff', fontSize: hp('3%'), marginLeft: '20%', marginTop: -2, textAlign: 'center' }}>
                                    {textNavegacion}
                                </CustomText>
                            </View>
                        </TouchableHighlight>
...
RNParallax.propTypes = {
    ...
    headerNavbarStyle: PropTypes.any,
};
